...
def run_pipeline(analysis, db) -> PipelineOutput:
    ...
    for img in images:
        ...
        # --- read + validate ---
        try:
            data = image_service.download_image_bytes(img.file_path)
    ...
    t_pipeline_ms = int((time.perf_counter() - t_pipeline_start) * 1000)
    ocr_ms = sum(t["ocr_ms"] for t in out.timings)
    proc_ms = sum(t["processing_ms"] for t in out.timings)
    logger.info(
        "OCR pipeline %s: total=%dms ocr=%dms processing=%dms images=%d",
        analysis.id, t_pipeline_ms, ocr_ms, proc_ms, len(images),
    )
