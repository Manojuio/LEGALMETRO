...
def run_complete_analysis(analysis: Analysis, db) -> dict:
    ...
    # 3. Classification — respect the inspector's manually-selected category.
    t0 = time.perf_counter()
    ...
    classification = classification_service.classify(
        ...
        raw_text,
        existing_category=analysis.category,
    )
    ...
    # 8. Overall status is derived from the real weighted score
    ...
    logger.info(
        "Pipeline timing: total=%dms ocr=%dms classify=%dms",
        t_total_ms,
        int(t_ocr * 1000),
        int(t_classify * 1000),
    )

    # 10. Build response with scoring
    return {
        "analysis_id": analysis.id,
        "product": {
