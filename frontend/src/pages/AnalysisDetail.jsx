...
export default function AnalysisDetail() {
  ...
  // Load stored results for a completed analysis (READ-ONLY, never re-runs OCR).
  useEffect(() => {
    if (report) return
    let cancelled = false
    async function loadStored() {
      setLoading(true)
      try {
        const stored = await api.getResult(id)
        if (!cancelled) setReport(stored)
      } catch (err) {
        // No stored result yet (409) or not accessible — silently allow manual run.
      } finally {
    ...
    <div>
      ...
      {loading && !report && (
        <section className="panel">
          <div className="empty">
            <p className="muted">Loading stored results…</p>
          </div>
        </section>
      )}
                ...
                <table className="rule-table">
                  ...
                  <tbody>
                    {report.rules.map((r, i) => (
                      <tr key={i}>
                        <td className="col-num mono">{i + 1}</td>
                        <td className="col-title">
                          <span className="rule-name">{r.title}</span>
                          {r.rule && (
                            <span className="rule-internal mono">LM-{r.rule}</span>
                          )}
                        </td>
